...
Create a report per the `# Creating a Change Report` and `# Report Style (Default)` sections of the global instructions. Report only — do not perform a cleanup review.
...
Produce two outputs with the same title: a detailed report saved into the vault under `reports/`, and a concise console report following the `# Response and Report Format` section of the global instructions.